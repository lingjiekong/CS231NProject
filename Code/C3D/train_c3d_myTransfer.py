...
def tower_acc(logit, labels):
  correct_pred = tf.equal(tf.argmax(tf.argmax(logit, axis=0), axis=1), labels)
...
def run_training():
  ...
  with tf.Graph().as_default():
    ...
    for gpu_index in range(0, gpu_num):
      with tf.device('/gpu:%d' % gpu_index):
        with tf.name_scope('%s_%d' % ('Infiniti-C3D', gpu_index)) as scope:
          ...
          myWeights = _variable_with_weight_decay('wout', [4096, c3d_model_transfer.NUM_CLASSES], 0.000)
    ...
    init = tf.variables_initializer([myWeights, myBiases])
    # train_op = tf.group(opt1.minimize(loss, var_list=[myWeights, myBiases]), opt2.minimize(loss, var_list=[myWeights, myBiases]))
    train_op = opt1.minimize(loss, var_list=[myWeights, myBiases])
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      if (step) % 5 == 0 or (step + 1) == FLAGS.max_steps:
        saver.save(sess, os.path.join(model_save_dir, 'my_c3d_model'), global_step=step)
